...
def generate_line_shaded(df, title):
    ...
    fig.update_layout(
        ...
        title={'text':f'<b>Growth of Capital {title}</b>', 'x':0.5, 'y':0.98, 'font':{'size':16}},
...
def generate_weekly_bars(df):
    dfW = df.resample('W').agg({'pnl_ac':'sum'})
    bars = px.bar(dfW, x=dfW.index, y=['pnl_ac'],
...
def generate_monthly_bars(df):
    dfM = df.resample('M').agg({'pnl_ac':'sum'})
    bars = px.bar(dfM, x=dfM.index, y=['pnl_ac'],
...
def generate_QPtable(df):
    ...
    dfQ['pnl_ac'] = dfQ['pnl_ac'].map(lambda x: f"{x:.2%}")
    
    dftable = dfQ.pivot(index='Year', columns='Quarter', values='pnl_ac')
...
#       title = {'text': f'{figln_title} Sales vs Target'},
       delta = {'reference':  profit_target, 'valueformat': '.2%'},
       gauge = {'axis': {'range': [None, profit_target * 1.35], 'tickformat':',.2%', 'tickvals':[0,0.12,0.3, 0.4]},
                'bar': {'color': bar_color},  
        #        'shape':'angular',
...
#       title = {'text': f'{figln_title} Sales vs Target'},
       delta = {'reference':  profit_target, 'valueformat': '.2%'},
       gauge = {'axis': {'range': [None, profit_target * 1.35], 'tickformat':',.2%', 'tickvals':[0,0.04,0.1,0.13]},
                'bar': {'color': bar_color},  
        #        'shape':'angular',
                'steps' : [{'range': [0, profit_target * 1.35], 'color': '#FFFFFF'},],
                'threshold' : {'line': {'color': 'red', 'width': 4}, 'thickness': 0.75, 'value': profit_target},
